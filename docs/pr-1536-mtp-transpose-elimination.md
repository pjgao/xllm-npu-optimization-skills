...
**重要更正**：PR #1536 当前代码（`1d07999f`）没有实现 `causal_conv1d` 复用，仍然调用 `run_spec_verify_conv()` / `causal_conv1d_update`。本文中的“复用 `causal_conv1d`”只能作为后续方案或新 PR 方向，不能写成 PR #1536 已完成的优化点。
