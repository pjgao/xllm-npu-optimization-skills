...
torch::Tensor run_spec_verify_conv(const torch::Tensor& mixed_qkv,
  ...
  const int64_t channels = mixed_qkv.size(2);
...
torch::Tensor Qwen3GatedDeltaNetBaseImpl::forward(
  ...
  if (!use_spec_verify && is_any_prefill) {
    ...
    if (!conv_weight_transposed_.defined()) {
      conv_weight_transposed_ = conv_weight.transpose(0, 1).contiguous();
    }
    mixed_qkv =
        run_spec_verify_conv(mixed_qkv,
                             conv_cache,
                             logical_state_indices,
                             input_params.num_accepted_tokens.to(device),
                             attn_metadata.q_cu_seq_lens,
                             conv_weight_transposed_,
                             conv_kernel_size_);
  } else {
...
Qwen3GatedDeltaNetBaseImpl::process_mixed_qkv(torch::Tensor& mixed_qkv) const {
  ...
  bool is_btc_format = (channels != (k_size_ * 2 + v_size_) / tp_size_);
  if (!is_btc_format) {
    mixed_qkv = mixed_qkv.transpose(1, 2);
  }
